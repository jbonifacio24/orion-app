...
class _CreateProductPageState extends State<CreateProductPage> {
  ...
  @override
  Widget build(BuildContext context) { final categories = context.watch<CategoriesCubit>().state; return BlocListener<ProductFormCubit, ProductFormState>(listenWhen: (previous, current) => !previous.success && current.success, listener: (context, state) => context.goNamed('my-products'), child: Scaffold(appBar: AppBar(title: const Text('Crear producto')), body: categories is CategoriesLoaded ? SingleChildScrollView(padding: const EdgeInsets.all(20), child: BlocBuilder<ProductFormCubit, ProductFormState>(builder: (context, form) => ProductForm(categories: categories.categories, initial: null, loading: form.submitting, onSubmit: context.read<ProductFormCubit>().submit))) : Center(child: categories is CategoriesFailure ? Text(categories.message) : const CircularProgressIndicator()))); }
}
